...
const JITO_RPC_URL = "https://frankfurt.mainnet.block-engine.jito.wtf/api/v1/bundles";
...
async function createJitoBundle(transaction, wallet, connection) {
    try {
  ...
  async function sendJitoBundle(bundle) {
    try {
        ...
        // Assign bundleSignature here if successful
        bundleSignature = response.data.result;
        console.log(`✅ Jito Bundle Sent! Signature: ${bundleSignature}`);
 ...
 async function checkBundleStatus(bundleId) {
    try {
      ...
      const transactionId = result.transactions.length > 0 ? result.transactions[result.transactions.length - 1] : null;
  ...
  module.exports = { createJitoBundle, sendJitoBundle, bundleSignature, checkBundleStatus };